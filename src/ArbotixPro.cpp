 ...
 *   ArbotixPro.cpp
 *
 *   Author: ROBOTIS, Interbotix, EARL Tech.
 *
 */
#include <stdio.h>
#include <stdlib.h>

#include "/home/gregorio626/EARL/Framework/include/dynamixel/PRO/ArbotixPro.h"
...
BulkReadData::BulkReadData() :
	...
	error(-1)
{
	for (int i = 0; i < AXServo::MAXNUM_ADDRESS; i++)
		table[i] = 0;
}
...
/*
	if (m_bIncludeTempData == true)
		{
			for (int id = DXL_Data::ID_MIN; id <= DXL_Data::ID_MAX; id++)
				{
					if (MotionStatus::m_CurrentJoints.GetEnable(id))
						{
							...
							m_BulkReadTxPacket[PARAMETER + 3 * number + 3] = AXServo::P_PRESENT_TEMPERATURE; // start address
							number++;
						}
				}
		}
